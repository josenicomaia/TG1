...
session_start();
    ...
    $resultado = $mysqli->query("SELECT propostas.*, clientes.RazaoSocial FROM propostas INNER JOIN clientes ON propostas.cliente_uid = clientes.uid_cliente where propostas.estagio <=  3 and date(`datacriacao`) between date('$datacriacao') and date('$datacriacaofinal')");
